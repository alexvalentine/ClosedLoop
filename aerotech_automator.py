...
class AerotechAutomator(object):
    ...
    def setup(self):
        ...
        self.km = KeyenceMicrometer('COM5')
    ...
    def zero_nozzle(self, axis):
        ...
        sweep_range = 4  #1.5# length of sweep range for z-measure (mm)
        sweep_speed = 0.2  #0.1# speed of sweep for z-measure (mm/s)
    ...
    def profile_line_csv_output(self,spacing,dist):
    ...
    def rezero_nozzles(self, nozzles, alignment_path = None, cal_file = False):
        ...
        if cal_file is not False:
            self.write_master_cal_file()
            self.g.abs_move(y=self.substrate_origins['slide1']['A'][1]-2)
            self.g.feed(5)
            self.g.set_cal_file(self.calfile_path)
